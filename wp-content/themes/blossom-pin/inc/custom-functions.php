...
function blossom_pin_scripts() {
    ...
    wp_enqueue_script( 'blossom-pin', get_template_directory_uri() . '/js' . $build . '/custom' . $suffix . '.js', array( 'jquery', 'masonry' ), BLOSSOM_PIN_THEME_VERSION, true );
...
function blossom_pin_change_comment_form_default_fields( $fields ){    
    ...
    $fields['author'] = '<p class="comment-form-author"><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'blossom-pin' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'blossom-pin' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['url'] = '<p class="comment-form-url"><input id="url" name="url" placeholder="' . esc_attr__( 'Website', 'blossom-pin' ) . '" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>'; 
...
function blossom_pin_change_comment_form_defaults( $defaults ){    
    $defaults['comment_field'] = '<p class="comment-form-comment"><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comment', 'blossom-pin' ) . '" cols="45" rows="8" aria-required="true"></textarea></p>';
...
function blossom_pin_get_the_archive_title( $title ){
    ...
    if( is_post_type_archive( 'product' ) ){
        $title = sprintf( __( '%1$s%2$s%3$s', 'blossom-pin' ), '<h1 class="pate-title">', get_the_title( get_option( 'woocommerce_shop_page_id' ) ), '</h1>' );
    }
